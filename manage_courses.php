<?php 
    ...
    if($action=="addClass"){
        ...
        $query = "insert into courses values($id_for_class,'$class')";
        $query_2 = "insert into manage_courses values($id_for_class,$Aid)";
      ...
      $result = mysqli_query($connection,$query);
        ...
        $id_for_class = $_POST["id_for_class"];
        $id_for_class = intval($id_for_class);
        $query = "delete from courses where Course_id=$id_for_class";
        ...
        $id_for_class = $_POST["id_for_class"];
        $id_for_class = intval($id_for_class);
        $query = "select Name from courses where course_id=$id_for_class";
        $result = mysqli_query($connection,$query);
        if(mysqli_num_rows($result)>0){
          foreach($result as $row){
              $name = $row["Name"];
          }
            echo "Course name ".$name." with id ".$id_for_class;
                $connection->close();
                echo "<br><br><br><br>";
                echo "<a href='admin_panel.php'>Click here to go back admin panel</a>";          }           
        else{
